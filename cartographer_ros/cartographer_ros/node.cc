  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (trajectory_data.local_slam_data->time !=
      ...
      if (scan_matched_point_cloud_publisher_.getNumSubscribers() > 0) {
        ...
        point_cloud.reserve(trajectory_data.local_slam_data->range_data_in_local
                                .returns.size());
        // TODO: 获取local_slam_data的点云数据, 填入到point_cloud中
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      if (node_options_.publish_to_tf) {
        ...
        } else {  // cartographer不需要提供odom坐标系,则发布 map_frame -> published_frame 的tf
          stamped_transform.header.frame_id = node_options_.map_frame;
          stamped_transform.child_frame_id =
              trajectory_data.trajectory_options.published_frame;
          stamped_transform.transform = ToGeometryMsgTransform(
...
int Node::AddTrajectory(const TrajectoryOptions& options) {
  ...
  // 新增一个位姿估计器: extrapolators_
  AddExtrapolator(trajectory_id, options);
  // 新生成一个传感器数据采样器: sensor_samplers_
...
bool Node::HandleStartTrajectory(
  ...
  // 如果给定了一个初始位姿 Check 1. initial_pose 2. status of the specific trajectory_id defined in request; 
  ...
  } else if (!ValidateTopicNames(trajectory_options)) {  // 检查topic名字是否被其他轨迹使用
